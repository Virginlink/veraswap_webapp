...
class ImportLiquidity extends Component {
    ...
    importPool = () => {
        const { tokenA, tokenAIcon, tokenAAddress, tokenB, tokenBIcon, tokenBAddress, lpAddress } = this.state
    ...
    render() {
        const { loading, poolFound, tokenAModalVisible, tokenBModalVisible, tokenA, tokenB, tokenAIcon, tokenBIcon, tokenAAddress, tokenBAddress, lpAddress } = this.state
        const { onModalToggle, walletConnected, walletAddress, history, modalVisible, theme, onThemeToggle, ethBalance, vrapBalance } = this.props
        return (
            <>
                <Navbar
                        ...
                        <div className="exchange-card">
                            <div className="pool-form">
                                <div className="flex-spaced-container" style={{marginBottom: '1.5rem', color: theme === 'light' ? '#000' : '#FFF'}}>
                                    ...
                                    <Tooltip title="Use this tool to find pairs that don't automatically appear in the interface." placement="bottom">
                                        <AiOutlineQuestionCircle size={16} />
                                    </Tooltip>
                                </div>
                                <button className="import-pool-select" style={{marginBottom: '1rem'}} onClick={this.toggleModalA}>
                                    <div>
                                ...
                                <div className="liquidity-section">
                                    {(walletConnected && walletAddress) ? loading ? (
