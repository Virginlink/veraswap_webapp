...
class Exchange extends Component {
  ...
  swapTokensInternal = () => {
    const { tokenA, tokenAIcon, tokenB, tokenBIcon, tokenBBalance, tokenAAddress, tokenBAddress } =
      this.state;
  ...
  calculatePriceImpact = () => {
    ...
    const numerator =
      parseFloat(tokenASupply) * parseFloat(tokenBAmount) +
      parseFloat(tokenBSupply) * parseFloat(tokenAAmount);
    const denominator =
      parseFloat(tokenBSupply) * (parseFloat(tokenBSupply) + parseFloat(tokenBAmount));
              ...
              if (res.data.hash) {
                const Link = () => (
                  <a
                    ...
                    href={`https://testnet.bscscan.com/tx/${res.data.hash}`}
                  >
                    View Transaction
                  </a>
                );
                  ...
                  !tokenAAmount || !tokenBAmount ? (
                    ...
                    parseFloat(tokenAAmount) <= parseFloat(tokenAAllowance) ? (
                      parseFloat(impact) <= parseFloat(this.context.slippage) ? (
                        <div className="exchange-button-container">
                          <button onClick={this.swap} disabled={loading || swapping}>
                            Swap{" "}
                            {swapping && (
                              <CircularProgress
                                size={12}
